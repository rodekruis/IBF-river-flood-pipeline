...
TRIGGER_THRESHOLDS = [1.5, 2, 5, 10, 20, 50, 100, 200, 500]
...
class BaseDataSet:
    ...
    def get_data_unit(self, pcode: str, lead_time: int):
        ...
        bdu = next(filter(lambda x: x.pcode == pcode and x.lead_time == lead_time, self.data_units), None)
        if not bdu:
            return None
        else:
            return bdu
    ...
    def upsert_data_unit(self, data_unit: BaseDataUnit):
        ...
        bdu = next(filter(lambda x: x[1].pcode == data_unit.pcode and x[1].lead_time == data_unit.lead_time,
                          enumerate(self.data_units)), None)
        if not bdu:
